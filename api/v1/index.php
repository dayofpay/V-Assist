...
// $captchaResponse = file_get_contents("https://www.google.com/recaptcha/api/siteverify?secret=<SECRET_TOKEN>&response=" . $captcha . "&remoteip=" . $_SERVER['REMOTE_ADDR']);
...
if($_POST['serviceType'] == "checkGlobi"){
  ...
  if(isset($_POST['personalId']) && isset($_POST['licenseId'])) {
    $url = "https://e-uslugi.mvr.bg/api/Obligations/AND?mode=1&obligedPersonIdent=" . $_POST['personalId'] . "&drivingLicenceNumber=" . $_POST['licenseId'];
  }
  ...
  $data = $response;
      if($response !== false) {
          if(str_contains($data,'"hasNonHandedSlip":false')){
          ...
          else if(str_contains($data,'"hasNonHandedSlip":true')){
          ...
          if(str_contains($data,"obligedPersonName")){
            echo '<div class="alert alert-dark alert-icon alert-dismissible fade show" role="alert">
            <i class="uil uil-receipt-alt"></i> Намерени са неплатени задължения <br> '. 
            '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
          </div>';
          ...
          echo "<tbody>\n";
          foreach ($jsonData['obligations'] as $obligation) {
              echo "<tr>\n";
